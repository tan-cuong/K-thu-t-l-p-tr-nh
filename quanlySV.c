...
void selection_Sort(SinhVien ds[], int n){
    int min_idx;
    for (int i=0; i<n-1; i++){
        min_idx=i;
        for(int j=i+1; j<n; j++){
            if(ds[j].dtb<ds[min_idx].dtb){
                min_idx=j;
            }
        }
        if(min_idx!=i){
            SinhVien temp=ds[min_idx];
            ds[min_idx]=ds[i];
            ds[i]=temp;
        }
    }
}
void insertion_Sort(SinhVien ds[], int n){
    SinhVien key;
    int j;
    for(int i=1; i<n; i++){
        key=ds[i];
        j=i-1;
        while(j>=0 && ds[j].dtb<key.dtb){
            ds[j+1]=ds[j];
            j=j-1;
        }
        ds[j+1]=key;
